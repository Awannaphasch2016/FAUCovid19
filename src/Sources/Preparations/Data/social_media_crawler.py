...
from global_parameters import ALL_KEYWORDS
...
def _get_running_conditions(crawler_option: str,
    ...
    :param search_type: search_type specify how (where) you want crawler to crawl (largely depends on api used by each crawler option)

    :type respond_type: str
    :param respond_type:  respond_type specify types of respond you wants (any functions can be applied here eg. find average tweet of 5 day by username start with James etc)
...
def run_crawler(run_crawler_func: Callable, crawler_condition: Union[
    ...
    """FIXME: this function should be removed and simply replaced by run_crawler_func()"""
    return run_crawler_func(crawler_condition)


def get_keywords_collections(crawler_type: str) -> Tuple[str]:
    """
    return aspects and list of keywords (query) for each aspects with respect to each crawler type
...
def twitter_crawler_condition(
    ...
    :param timestamp: timestamp at the time that data is retreived (not when data is published)

    :type running_conditions: RunningConditions
    :param running_conditions: a dict with key specify common conditions (constraints) for all crawlers
    ...
    (
        ...
        social_distance_keywords, lockdown_keywords,
        reopen_keywords) = get_keywords_collections('twitter')
...
def reddit_crawler_condition(
    ...
    (
        ...
        covid_keywords, reopening_keywords) = get_keywords_collections(
        'reddit')
    ...
    if collection_name == 'corona_general':
        ...
        subreddit_collection_class = {
            'collection': {'subreddit': General, 'aspect': tag,
                           'query': tag_words},
            'name': collection_name}
        ...
        subreddit_collection_class = {
            'collection': {'subreddit': Country, 'aspect': tag,
                           'query': tag_words},
            'name': collection_name}
        ...
        subreddit_collection_class = {
            'collection': {'subreddit': Region, 'aspect': tag,
                           'query': tag_words},
            'name': collection_name}
        ...
        subreddit_collection_class = {
            'collection': {'subreddit': states_subreddit, 'aspect': tag,
                           'query': tag_words},
            'name': collection_name}
...
def get_reddit_running_conditions(max_after: int, tags: Tags,
                                  crawler_option: Crawler_type) -> Union[
    List, List[RunningConditionsKeyValue]]:
    """
    prepare and return list of running condition (common constraints among crawler) for reddit crawler
    ...
    :return:  list of running condition (common constraints among crawler) for reddit crawler
    """
...
def get_twitter_running_conditions(max_after: int, tags: Tags,
                                   crawler_option: Crawler_type) -> Union[
    List, List[RunningConditions]]:
    """
    prepare and return list of running condition (common constraints among crawler) for twitter crawler
    ...
    :rtype: list of RunningConditionsKeyValue
    :return:  list of running condition (common constraints among crawler) for twitter crawler
    """
...
def get_crawler_running_conditions(max_after: int, tags: Tags,
                                   crawler_type: Crawler_type) -> Union[
    List, List[RunningConditions]]:
    """
    prepare and return list of running condition (common constraints among crawler) for a specified crawler
    ...
    :rtype: list of RunningConditionsKeyValue
    :return:  list of running condition (common constraints among crawler) for specified crawler
    """
...
def run_all_reddit_conditions(before_date: datetime.datetime,
                              after_date: datetime.datetime, max_after: int,
                              tags: Tags, timestamp: datetime.datetime,
...
def run_all_twitter_conditions(before_date: datetime.datetime,
                               after_date: datetime.datetime, max_after: int,
                               tags: Tags, timestamp: datetime.datetime,
                               crawler_type: Crawler_type) -> None:
    """
...
def run_all_conditions(before_date: datetime.datetime,
                       after_date: datetime.datetime, max_after: int,
                       tags: Tags, crawler_type: Crawler_type):
    """
    ...
    if crawler_type == 'reddit':
        run_all_reddit_conditions(before_date, after_date, max_after, tags,
                                  timestamp, crawler_type)
    elif crawler_type == 'twitter':
        run_all_twitter_conditions(before_date, after_date, max_after, tags,
                                   timestamp, crawler_type)
...
@click.option('--crawler_type',
              type=click.Choice(['reddit', 'twitter', 'all']))
# @click.argument('tags', nargs=-1)
@my_timer
def main(select_all_conditions: bool, tags: Tags, max_after: int,
         crawler_type: str, before_date: datetime.datetime,
    ...
    if select_all_conditions:
        run_all_conditions(before_date, after_date, max_after, tags,
                           crawler_type)
    else:
        raise NotImplementedError

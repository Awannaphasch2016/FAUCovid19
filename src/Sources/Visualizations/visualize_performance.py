...
class PlotPerformance:
    ...
    def __init__(self, is_plotted, is_saved_plot, **kwargs):
        ...
        3. plot_performance = PlotPerformance(
            ...
            plot_performance.plot_using_list_of_name((2,3),
             name_and_tuple_dict, fil_name, title,save_path)

        """
        assert isinstance(is_saved_plot, bool), (
            "please specified save_plot attribute in "
            "PlotClass to be boolean"
        )
        assert isinstance(is_plotted, bool), "is_plotted must have type = bool"

        self.hist = {}
        self.plt = plt
        self.is_saved_plot = is_saved_plot

    def collect_hist_using_list_of_name(
        self,
        dict_for_plot,
    ):
        """SKipped."""
    ...
    def plot_each_hist(self, ax_tuple=None, name=None):
        ...
        if self.row_col is not None:
            if self.row_col[0] == 1 and self.row_col[1] == 1:
                ...
                # self.axs[ax_tuple[0]][ax_tuple[1]].set(xlabel='epochs',
                #                                        ylabel='val',
                #                                        title=name)
